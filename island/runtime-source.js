...
await entryStage(13,'Finishing the northern neighbourhood');
  ...
  const parkEdgePatch=await islandFetch('/67park-kimi-party/repairs/park-edges.json').then(r=>{if(!r.ok)throw Error('Park edge repair missing');return r.json();});
  renderer.domElement.dataset.parkEdges=JSON.stringify(applyParkEdges(kok,parkEdgePatch));
  const curbJoinPatch=await islandFetch('/67park-kimi-party/repairs/curb-joins.json').then(r=>{if(!r.ok)throw Error('Curb join repair missing');return r.json();});
