...
export async function loadLunapark77({scene,renderer,sample,sea,variant,terrainRoot,timberSample}){
 ...
 await Promise.all(Array.from({length:4},async()=>{
  while(nextAsset<queue.length){const asset=queue[nextAsset++],articulated=['ferris','carousel','carouselSmall'].includes(asset);
   ...
   assets.set(asset,(await loader.loadAsync('/67park-kimi-party/island/lunapark-v1/'+asset+'.glb?v='+version)).scene);
  }
 }));
 for(const p of layout77){
  ...
  if(!assets.has(p.asset))assets.set(p.asset,(await loader.loadAsync('/67park-kimi-party/island/lunapark-v1/'+p.asset+'.glb?v='+assetVersion)).scene);
